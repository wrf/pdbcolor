...
par(mar=c(6,4.5,3,1))
plot(xpositions, first_v_second_lnl, type="h", pch=16, col=colorset[rowmaxs], lwd=150/numgenes, xlim=range(xpositions), ylim=c(0,max(pretty(ymax))), ylab=expression(paste(Delta,"ln(L)",sep="")), xlab="", main=mainlab, axes=FALSE, cex.lab=1.4)
axis(2, cex.axis=1.3)
